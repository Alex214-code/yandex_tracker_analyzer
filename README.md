...
1. **Первоначальная настройка** (опционально, один раз):
   - Откройте Swagger UI: `http://localhost:8000/docs`
   - Перейдите в раздел "Проекты"
   - Посмотрите текущий список проектов по умолчанию, которые будут использоваться для генерации Excel-файла: `GET /projects/default`
   - Если нужны другие проекты, вызовите `GET /projects/available`, чтобы получить список всех доступных проектов
   - Выберите нужные проекты и сохраните их через `PUT /projects/default`

   > Если встроенные проекты вас устраивают, настройка не требуется — сразу переходите к генерации отчёта.
...
3. **Изменение списка проектов** (при необходимости):

   Список проектов можно изменить в любой момент через Swagger UI — см. раздел [API → Проекты](#проекты).
...
3. Значения из конфигурации: `TARGET_PROJECTS` из `.env`, или встроенные значения из кода (если переменная не задана)
