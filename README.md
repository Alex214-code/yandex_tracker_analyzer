...
1. **Настройка проектов** (опционально, один раз):
   - Откройте Swagger UI: `http://localhost:8000/docs`
   - Посмотрите текущий список проектов: `GET /projects/default`
   - Если нужны другие проекты — настройте их через `PUT /projects/default`

   > Подробнее см. раздел [Управление проектами](#управление-проектами).
...
**Преимущество**: Список проектов сохраняется между сессиями — для генерации отчёта достаточно указать только период.
...
### Приоритет источников при генерации отчёта

1. Явно указанные в запросе `POST /reports/generate` (параметр `projects`)
2. Сохранённые через `PUT /projects/default` (файл `user_settings.json`)
3. Встроенные значения из кода (см. ниже)

### Встроенные значения из кода

Определены в `src/settings.py`:
